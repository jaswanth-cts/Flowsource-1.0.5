...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-cicd-gcp/';
    ...
    async function getVersion() {
        try 
        {
            ...
            const value = JSON.parse(sessionStorage.getItem('GcpCiCdBackendVersion'));
            ...
            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('GcpCiCdBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('GcpCiCdBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('GcpCiCdBackendVersion', '-');
