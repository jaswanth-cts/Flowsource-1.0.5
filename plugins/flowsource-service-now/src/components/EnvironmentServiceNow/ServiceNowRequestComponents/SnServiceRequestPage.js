...
const SnServiceRequestPage = ({ environmentConfigItem }) => {
  ...
  const handlePageChange = page => {
    setLoading(true);
    fetchServiceRequests(page, itemsPerPage);
  };

  const fetchServiceRequests = async (pageNumber, pageSize) => {
    try {
      const url = `${backendBaseApiUrl}snow-service-request?pageNumber=${encodeURIComponent(pageNumber)}
        &pageSize=${encodeURIComponent(pageSize)}
        &configurationItem=${encodeURIComponent(environmentConfigItem)}`;
      ...
      if (response.ok) {
        ...
        if (snowResult) {
          if (snowResult.snowServiceReq && snowResult.snowServiceReq.length <= 0) {
            setpaginationData([]);
            ...
            setpaginationData(snowResult.snowServiceReq);
            setCurrentPage(pageNumber);
  ...
  useEffect(() => {
    fetchServiceRequests(1, itemsPerPage);
  }, []);

  function renderServiceNowOrdersPage() {
    if (isLoading) {
      return (
        ...
        <div className={`card`}>
          <div className={`card-body`}>
            <div className={`${classes.pluginHeading}`}>
              <div className={`${classes.header}`}> Service Request </div>
              <div style={{ marginBottom: '1rem' }}>
                <PluginVersion />
              </div>
            </div>
            {renderServiceNowOrdersPage()}
          </div>
        </div>
      </div>
    </div>
...
export default SnServiceRequestPage;