...
const SnProvisionOrderInfraProvisionPage = () => {
  ...
  const fetchProvisionOrders = async (pageNumber, pageSize) => {
    try 
    {
      const url = `${backendBaseApiUrl}infraprovision-snow-provision-orders?pageNumber=${encodeURIComponent(pageNumber)}
      ...
      if (response.ok) {
        ...
        if (snowResult) {
          if (
            snowResult.snowDetailsObject &&
            snowResult.snowDetailsObject.length <= 0
          ) {
            setpaginationData([]);
            ...
            setpaginationData(snowResult.snowDetailsObject);
            setCurrentPage(pageNumber);
...
export default SnProvisionOrderInfraProvisionPage;