...
const SnOrdersPage = ({ configurationItem }) => {
  ...
  const handlePageChange = page => {
    setLoading(true);
    fetchOrdersInfo(page, itemsPerPage);
  };

  const fetchOrdersInfo = async (pageNumber, pageSize) => {
    try {
      const url = `${backendBaseApiUrl}snow-orders?pageNumber=${encodeURIComponent(pageNumber)}
      ...
      if (response.ok) {
        ...
        if (snowResult) {
          if (snowResult.snowOrders && snowResult.snowOrders.length <= 0) {
            setpaginationData([]);
            ...
            setpaginationData(snowResult.snowOrders);
            setCurrentPage(pageNumber);
        ...
        if (response.status === 503) {
          ...
          setError(`HTTP Status: ${response.status}, Message: ${errorText.error}`);
        }
      }
      setLoading(false);
    } catch (error) {
  ...
  useEffect(() => {
    fetchOrdersInfo(1, itemsPerPage);
  }, []);
