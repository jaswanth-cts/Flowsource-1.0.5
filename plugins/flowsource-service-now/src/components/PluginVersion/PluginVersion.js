...
const PluginVersion = () => {
    ...
    async function getVersion() {
        try {
            ...
            const value = JSON.parse(sessionStorage.getItem('ServiceNowBackendVersion'));

            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                setBackendVersion(value);
                log.info('Backend version from session storage: ' + value);
            } else {
                log.info('Backend version from fetch');
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('ServiceNowBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('ServiceNowBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('ServiceNowBackendVersion', '-');
