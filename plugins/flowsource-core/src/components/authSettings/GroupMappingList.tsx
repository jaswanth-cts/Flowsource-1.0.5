...
export default function GroupMappingList({setShowList}:any) {

    const classes = useStyles();
    const config = useApi(configApiRef);
    const { fetch } = useApi(fetchApiRef);
    const roleMappingUrl = config.getString('backend.baseUrl') + `/api/flowsource-core/groups`
    const roleDeletUrl = config.getString('backend.baseUrl') + `/api/flowsource-core/role-delete-by-id`
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const url = roleMappingUrl + `?_page=${page + 1}&_pageSize=${rowsPerPage}`;
      ...
      <>
        ...
        <Box sx={{
          ...
          try{
            ...
            const response = await fetch(roleDeletUrl, {
              method: 'DELETE',
            ...
            <TableBody>
              {groupsData.map((row)=>(
                <TableRow key={row.ID}>
                      ...
                      <TableCell>{row.flowsource_role}</TableCell>
                      <TableCell>{row.auth_provider}</TableCell>
