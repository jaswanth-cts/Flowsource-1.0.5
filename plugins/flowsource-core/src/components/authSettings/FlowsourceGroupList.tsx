...
export default function FlowsourceGroupList({setShowList}:any) {

    const classes = useStyles();
    const config = useApi(configApiRef);
    const { fetch } = useApi(fetchApiRef);
    const flowsourceGroupUrl = config.getString('backend.baseUrl') + `/api/flowsource-core/get-flowsource-groups`
    const deleteUrl = config.getString('backend.baseUrl') + `/api/flowsource-core/delete-flowsource-group`
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const url = flowsourceGroupUrl + `?_page=${page + 1}&_pageSize=${rowsPerPage}`;
