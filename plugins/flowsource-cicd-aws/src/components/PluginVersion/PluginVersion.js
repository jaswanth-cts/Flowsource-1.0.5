...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-cicd-aws/';
    ...
    async function getVersion() {
        try 
        {
            ...
            const value = JSON.parse(sessionStorage.getItem('AWSCiCdBackendVersion'));
            ...
            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('AWSCiCdBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('AWSCiCdBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('AWSCiCdBackendVersion', '-');
