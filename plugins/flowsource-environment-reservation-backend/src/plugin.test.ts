...
import { maintenanceRequestsPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [maintenanceRequestsPlugin],
    });

    await request(server).get('/api/maintenance-requests/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/maintenance-requests/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/maintenance-requests/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/maintenance-requests/todos/${createdTodoItem.id}`)
