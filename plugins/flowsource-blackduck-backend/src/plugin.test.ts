...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [flowsourceBlackduckPlugin],
    });

    await request(server).get('/api/flowsource-blackduck/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/flowsource-blackduck/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/flowsource-blackduck/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/flowsource-blackduck/todos/${createdTodoItem.id}`)
