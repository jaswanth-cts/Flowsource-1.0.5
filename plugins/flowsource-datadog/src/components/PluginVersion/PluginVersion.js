...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-datadog/';
    ...
    async function getVersion() {
        try 
        {
            ...
            const value = JSON.parse(sessionStorage.getItem('DataDogBackendVersion'));
            ...
            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('DataDogBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('DataDogBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('DataDogBackendVersion', '-');
