...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-playwright/';
    ...
    async function getVersion() {
        try 
        {
            ...
            const value = JSON.parse(sessionStorage.getItem('PlaywrightBackendVersion'));
            ...
            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('PlaywrightBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('PlaywrightBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('PlaywrightBackendVersion', '-');
