...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [flowsourceAzureReleasePlugin],
    });

    await request(server).get('/api/flowsource-azure-release/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/flowsource-azure-release/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/flowsource-azure-release/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/flowsource-azure-release/todos/${createdTodoItem.id}`)
