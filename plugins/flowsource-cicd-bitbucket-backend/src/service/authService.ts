...
export default class AuthService {
  ...
  /**
   * Resolve an auth token and api baseUrl for Bitbucket used by the CICD plugin.
   * Priority:
  ...
  async getAuthToken(
    ...
    if (clientId && clientSecret && authUrl) {
      ...
      if (!token || !baseUrl) {
        const err = new Error(
          'Bitbucket CICD plugin failed with error 503, missing values in bitbucketServer',
        );
