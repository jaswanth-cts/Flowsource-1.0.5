...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-blackduck/';
    ...
    async function getVersion() {
        try {
            ...
            if (frontEndPackageJson.version != null) {
                setFrontendVersion(frontEndPackageJson.version);
            } else {
                setFrontendVersion('-');
            }

            const value = JSON.parse(sessionStorage.getItem('BlackDuckBackendVersion'));

            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                setBackendVersion(value);
                console.log('Backend version from session storage: ' + value);
            } else {
                console.log('Backend version from fetch');
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null) {
                        setBackendVersion(backendVersionInfo.version);
                        sessionStorage.setItem('BlackDuckBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('BlackDuckBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('BlackDuckBackendVersion', '-');
