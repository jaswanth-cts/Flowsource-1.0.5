# flowsource-service-now
  ...
  Welcome to the flowsource-service-now backend plugin!
...
## Prerequisites
  ...
  2. Integrating required code for `Service Now Backend Plugin` to work with `Flowsource`.
...
### 3. Service Now:
  ...
  ```
    1.Add the below in packages/backend/package.json
      - "@flowsource/plugin-flowsource-service-now-backend": PLUGIN_VERSION (eg: "^1.0.0")
    
    2. Add the below in packages/backend/src/index.ts
