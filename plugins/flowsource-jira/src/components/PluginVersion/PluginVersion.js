...
const PluginVersion = () => {
    ...
    const backendBaseApiUrl = config.getString('backend.baseUrl') + '/api/flowsource-jira/';
    ...
    async function getVersion() {
        try 
        {
            ...
            const value = JSON.parse(sessionStorage.getItem('JiraBackendVersion'));
            ...
            if (value !== null && value !== 'null' && value !== undefined && value !== '-' && value !== '') {
                ...
                if (response.ok) {
                    ...
                    if (backendVersionInfo.version != null || backendVersionInfo.version != undefined) {
                        ...
                        sessionStorage.setItem('JiraBackendVersion', JSON.stringify(backendVersionInfo.version));
                    } else {
                        setBackendVersion('-');
                        sessionStorage.setItem('JiraBackendVersion', '-');
                    }
                } else {
                    setBackendVersion('-');
                    sessionStorage.setItem('JiraBackendVersion', '-');
