...
import { flowsourcePromptLibraryMetricsPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [flowsourcePromptLibraryMetricsPlugin],
    });

    await request(server).get('/api/flowsource-prompt-library-metrics/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/flowsource-prompt-library-metrics/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/flowsource-prompt-library-metrics/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/flowsource-prompt-library-metrics/todos/${createdTodoItem.id}`)
  ...
  it('should create TODO item with catalog information', async () => {
    const { server } = await startTestBackend({
      features: [
        flowsourcePromptLibraryMetricsPlugin,
