...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [flowsourcePdlcPlugin],
    });

    await request(server).get('/api/flowsource-pdlc/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/flowsource-pdlc/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/flowsource-pdlc/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/flowsource-pdlc/todos/${createdTodoItem.id}`)
