...
public class SampleController {
    ...
    public String slow() {
        ...
        log.info("Starting span for /slow endpoint");
        try (Scope scope = span.makeCurrent()) {
            String response = sampleService.slow();
            log.info("Response from slow service: {}", response);
            return response;
        } finally {
            span.end(); // End the span
            log.info("Ended span for /slow endpoint");
        }
    }
}
