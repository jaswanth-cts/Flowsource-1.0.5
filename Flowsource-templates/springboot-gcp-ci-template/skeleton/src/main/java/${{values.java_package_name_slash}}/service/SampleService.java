...
public class SampleService {
    ...
    public String slow() {
        Span span = Span.current();
        span.setAttribute("service.name", "slowService");
        log.info("Executing slow service");
        try {
    ...
    public String bulkheadFallback(Throwable ex) {
        Span span = Span.current();
        span.setAttribute("fallback.reason", "BulkheadException");
        log.warn("Fallback executed due to BulkheadException: {}", ex.getMessage());
        return "Too many requests - No further calls are accepted " + ex.getMessage();
    }
}